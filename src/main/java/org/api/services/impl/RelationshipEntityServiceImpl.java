...
public class RelationshipEntityServiceImpl implements RelationshipEntityService {
    ...
    @Override
    public ResultBean confirmOrUnConfirm(String json, String confirm) throws ApiValidateException, Exception {
        JsonObject jsonObject = DataUtil.getJsonObject(json);
        ValidateData.validate(ConstantJsonFileValidate.FILE_RELATIONSHIP_CONFIRM_JSON_VALIDATE, jsonObject, false);
        String idRelationship = DataUtil.getJsonString(jsonObject, ConstantColumns.ID_RELATIONSHIP);

        RelationshipEntity entity = relationshipEntityRepository.findOneById(idRelationship).orElseThrow(() -> new ApiValidateException(ConstantMessage.ID_ERR00005, ConstantColumns.ID_RELATIONSHIP,
                MessageUtils.getMessage(ConstantMessage.ID_ERR00005, null, ItemNameUtils.getItemName(ConstantColumns.ID_RELATIONSHIP, "Relationship"))));
