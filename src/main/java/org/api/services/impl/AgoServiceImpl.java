...
public class AgoServiceImpl implements AgoService {
    ...
    public ResultBean homePage(String idUser) throws ApiValidateException, Exception {
        ...
        List<UserHomeRespon> recommendFriends = recommendFriendsList.stream().map((relationshipEntity -> modelMapper.map(relationshipEntity, UserHomeRespon.class))).collect(Collectors.toList());

        for (UserHomeRespon friend: recommendFriends) {
            friend.setAvatars(getAvatarOrBanner(friend, "avatar"));
            friend.setBanners(getAvatarOrBanner(friend, "banner"));
        }
