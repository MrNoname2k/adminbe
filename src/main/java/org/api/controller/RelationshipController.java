...
import org.api.constants.ConstantRelationshipConfirm;
...
public class RelationshipController {
    ...
    @PostMapping(value = {"/confirm", "/unConfirm"}, produces = {MediaType.APPLICATION_JSON_VALUE})
    public ResponseEntity<ResultBean> confirmOrUnConfirm(@RequestBody String json, HttpServletRequest request) {
        try {
            ResultBean resultBean = null;
            if (request.getRequestURI().contains("/confirm")) {
                resultBean = relationshipEntityService.confirmOrUnConfirm(json, ConstantRelationshipConfirm.CONFIRM);
            } else if (request.getRequestURI().contains("/unConfirm")) {
                resultBean = relationshipEntityService.confirmOrUnConfirm(json, ConstantRelationshipConfirm.UN_CONFIRM);
            }
            return new ResponseEntity<ResultBean>(resultBean, HttpStatus.CREATED);
        } catch (ApiValidateException ex) {
            return new ResponseEntity<ResultBean>(new ResultBean(ex.getCode(), ex.getMessage()), HttpStatus.OK);
